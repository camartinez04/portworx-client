...
(function($) {
  ...
  $.ajax(settings).done(function (volumeUsage) {
    ...
    $(function() {
      ...
      if ($("#volume-utilization-chart").length) {
        ...
        var used = volumeUsage.volume_usage_percent
        var available =  volumeUsage.volume_available_percent
        var areaData = {
        ...
        var areaOptions = {
          ...
          legendCallback: function(chart) { 
            ...
            text.push('<div class="report-chart">');
              text.push('<div class="d-flex justify-content-between mx-4 mx-xl-5 mt-3"><div class="d-flex align-items-center"><div class="me-3" style="width:20px; height:20px; border-radius: 50%; background-color: ' + chart.data.datasets[0].backgroundColor[0] + '"></div><p class="mb-0">Utilized space</p></div>');
              text.push('<p class="mb-0">' + Math.round((volumeUsage.volume_usage/1024) * 100) / 100 + ' GB</p>');
              text.push('</div>');
              ...
              text.push('<p class="mb-0">' + Math.round((volumeUsage.available_space/1024) * 100) / 100 + ' GB </p>');
              text.push('</div>');
            text.push('</div>');
            return text.join("");
          },
        ...
        var volumeUtilizationChartPlugins = {
          beforeDraw: function(chart) {
            ...
            var text = volumeUsage.total_size/1024,
